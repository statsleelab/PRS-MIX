...
def parse_ldblk_mix(ldblk_dirs, mix_props, sst_dict, chrom):
    print('... parse mixed reference LD on chromosome %d ...' % chrom)

    # create a dictionary of SNPs and their flipping signs by SNP name
    flp_by_snp = {snp: flp for snp, flp in zip(sst_dict['SNP'], sst_dict['FLP'])}

    hdfs = []
    snp_blks_all = []

    # opening all reference panels
    for ref_dir in ldblk_dirs:
        if '1kg' in os.path.basename(ref_dir):
            # constructing file path for hdf5 file for given chromosome
            chr_name = ref_dir + '/ldblk_1kg_chr' + str(chrom) + '.hdf5'
        elif 'ukbb' in os.path.basename(ref_dir):
            chr_name = ref_dir + '/ldblk_ukbb_chr' + str(chrom) + '.hdf5'
        
        hdf_chr = h5py.File(chr_name, 'r')
        hdfs.append(hdf_chr)
        snp_blk = []
        # extracting the SNP list for each block in the hdf5 file
        for blk in range(1, len(hdf_chr) + 1):
            # decoding from bytes to string
            snp_blk.append([bb.decode("UTF-8") for bb in list(hdf_chr['blk_'+str(blk)]['snplist'])])
        # collecting SNP list per block for all the panels
        snp_blks_all.append(snp_blk)
        
    # number of blocks in this chromosome (each ethnicity can have different number of blocks so take the smallest one)
    n_blk = min(len(hdf) for hdf in hdfs)
    # store number of SNPs per block
    blk_sizes = []
    # to store the final mixed LD matrices
    ld_blk_mix = []
    # for each block intersect SNPS across ref panels and sum stats
    sst_snps = set(sst_dict['SNP'])

    # find SNPs that are common across ALL reference panels
    for blk in range(n_blk):
        common_snps = set(snp_blks_all[0][blk])
        # intersect with SNPs from all other panels
        for snp_blk in snp_blks_all[1:]:
            common_snps &= set(snp_blk[blk])
        # intersect again with summary stats SNPs
        common_snps &= sst_snps

        # if no common SNPs, save an empty matrix for this block and skip
        if not common_snps:
            ld_blk_mix.append(np.array([]))
            blk_sizes.append(0)
            continue

        # keep the SNP order from the first SNP panel
        snps_order = [s for s in snp_blks_all[0][blk] if s in common_snps]
        blk_size = len(snps_order)
        blk_sizes.append(blk_size)

        # build flip vector aligned to snps_order
        flip = np.array([flp_by_snp[snp] for snp in snps_order], dtype=float)
        F = np.outer(flip,flip)

        # weighted sum across all the LD panels
        R_mix = None
        for w, hdf_chr, snps_this in zip(mix_props, hdfs, snp_blks_all):
            # R_full is the full LD block matrix from one population's ref panel for blk
            R_full = np.array(hdf_chr[f'blk_{blk+1}']['ldblk'])
            snps_i = snps_this[blk]

            # reindex SNPs so that they are in the same order
            # pos_map is a dictionary of SNP names and their indices in snps_i
            pos_map = {snp: i for i, snp in enumerate(snps_i)}
            # pos is a list of indices of SNPs in snps_order
            pos = [pos_map[s] for s in snps_order]

            # slice R_sub as the LD submatrix for those SNPs containing only pos (basically LD matrix of intersected SNPs)
            # np.ix_() does advanced indexing, basically taking rows at pos and columns at pos (align both axes) - this is used in original PRScs function
            R_sub = R_full[np.ix_(pos,pos)]

            # apply allele flips using F matrix
            R_sub = R_sub*F

            # use SVD to symmetrize
            _, s, v = linalg.svd(R_sub)
            h = np.dot(v.T, np.dot(np.diag(s), v))
            R_sub = (R_sub + h) / 2

            # weighted sum across panels using mixing proportions (w = weights)
            # if it's the first LD matrix, initialize R_mix with the first one, otherwise keep accumulating the weighted sum
            R_mix = R_sub * w if R_mix is None else R_mix + R_sub * w
        ld_blk_mix.append(R_mix)

    for h in hdfs:
        h.close()
    return ld_blk_mix, blk_sizes
